...
bool approx_7A_with_5(uint8_t word)
{
    return (  I(F(word, 2) ^ F(word, 6))\
            - I(F(word, 1) ^ F(word, 2) ^ F(word, 3) ^ F(word, 4))\
            - I(F(word, 0) ^ F(word, 1))\
            + I(F(word, 3) ^ F(word, 4) ^ F(word, 5) ^ F(word, 6))\
            - I(F(word, 1) ^ F(word, 5) ^ F(word, 7))) >= 0;
}
