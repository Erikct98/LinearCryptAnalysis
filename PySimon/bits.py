...
def make_aes_sbox():
    def ls(x, steps):
        return (x << steps ^ x >> (8 - steps))

    def _sbox(x: int):
        return (x ^ ls(x, 1) ^ ls(x, 2) ^ ls(x, 3) ^ ls(x, 4) ^ 0x63) & 0xFF
    return _sbox
...
def S_table(s): return [[int(y) for y in x.strip().split(' ')]
                        for x in DESBOX_strings[s].strip(' ').split('\n')]
...
def example_sbox_2():
    SBOX = make_example_Sbox2(
        [8, 2, 4, 0, 15, 5, 7, 12, 10, 6, 11, 3, 14, 13, 9, 1], 2, 2)
...
def test_exercise_1():
    ...
    SBOX = make_example_Sbox2(
        [8, 2, 4, 0, 15, 5, 7, 12, 10, 6, 11, 3, 14, 13, 9, 1], k1, k2)
    ...
    counts = {0: list(approx.values()).count(
        0), 1: list(approx.values()).count(1)}
