...
void compute_correlation(uint32_t inp_mask, uint32_t out_mask, uint32_t sample_size, uint32_t nr_trials, uint8_t enc_rounds, uint32_t *results)
{
    ...
    for (int i = 0; i < nr_trials; i++)
    {
        ...
        subkey_approx = (subkeys[enc_rounds - 1] >> 12 ^ (subkeys[enc_rounds - 2] >> 11 & subkeys[enc_rounds - 2] >> 4)) & 1;
        ...
        for (int j = 0; j < sample_size; j++)
        {
            ...
            results[i] += ((ct >> 10 ^ ct >> 28) & 1) ^ subkey_approx;
        }
    }
}
...
int main()
{
    ...
    for (uint32_t sample_size = 0x100000; sample_size <= 0x100000; sample_size <<= 1)
    {
